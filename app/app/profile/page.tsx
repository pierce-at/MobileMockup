...
import { getTicketingIntegrationStatus } from "@/lib/integrations";
import { useAppState } from "@/lib/state/app-state";

const fields: ContactField[] = ["email", "linkedin", "website"];

const roleDestinations = {
  speaker: { href: "/app/workspace", label: "Speaker desk" },
...
export default function ProfilePage() {
  const ticketing = getTicketingIntegrationStatus();
  const {
    auth,
    currentUser,
    isSupabaseEnabled,
  ...
  const onboardingSteps = [
    {
      ...
      hint:
        auth.status === "authenticated"
          ...
          : "Use magic link so your week follows you."
    },
    {
      label: "Interests picked",
      done: currentUser.interests.length > 0,
      hint:
        currentUser.interests.length > 0
          ? `${currentUser.interests.length} interests selected`
          : "Choose topics to power recommendations."
    },
    {
      label: "First session saved",
      done: savedSchedule.length > 0,
      hint:
        savedSchedule.length > 0
          ? `${savedSchedule.length} saved in your week`
          : "Save one session so schedule and map feel personal."
    }
  ];
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[0.9fr,1.1fr]">
        <article className="space-y-4 rounded-[14px] border border-midnight/8 bg-white p-5 shadow-card">
          <div>
            <p className="text-xs uppercase tracking-[0.24em] text-coral">Identity</p>
            <div className="mt-4 flex items-center gap-3">
              <div className="grid h-14 w-14 place-items-center rounded-[10px] bg-midnight text-white">
                {currentUser.avatar}
              </div>
              <div>
          ...
          <div className="rounded-[12px] bg-mist p-4">
            <p className="text-xs uppercase tracking-[0.24em] text-coral">Onboarding</p>
            <p className="mt-2 font-display text-xl font-semibold text-midnight">
              {completedOnboardingSteps} of {onboardingSteps.length} basics ready
            </p>
            <div className="mt-3 grid gap-2">
              {onboardingSteps.map((step) => (
                <div
                  key={step.label}
                  className="flex items-start justify-between gap-3 rounded-[18px] bg-white px-3 py-3"
                >
                  <div>
                    <p className="text-sm font-semibold text-midnight">{step.label}</p>
                    <p className="text-xs leading-5 text-midnight/62">{step.hint}</p>
                  </div>
                  <span
          ...
          {roleLink ? (
            <Link
              href={roleLink.href}
              className="flex items-center justify-between rounded-[12px] border border-midnight/10 px-4 py-4 transition hover:border-midnight/20 hover:bg-mist"
            >
              <div>
          ...
          >
            <div>
              <p className="text-xs uppercase tracking-[0.24em] text-coral">Setup</p>
              <p className="mt-2 font-display text-xl font-semibold text-midnight">
                Onboarding page
              </p>
            </div>
        ...
        <article className="space-y-4 rounded-[14px] border border-midnight/8 bg-white p-5 shadow-card">
          <div>
            ...
            <div className="mt-4 rounded-[12px] border border-midnight/8 bg-[linear-gradient(180deg,#f9f7f0,#f2efe6)] p-4">
              ...
              <form onSubmit={handleSubmit} className="mt-4 flex flex-col gap-3 sm:flex-row">
                <input
                  ...
                  className="flex-1 rounded-[10px] border border-midnight/10 bg-white px-4 py-3 text-sm text-midnight outline-none transition focus:border-midnight/25"
                />
                <button
                  type="submit"
              ...
              {auth.status === "authenticated" ? (
                <button
                  ...
                  className="mt-3 rounded-[10px] border border-midnight/12 px-4 py-3 text-sm font-semibold text-midnight transition hover:bg-mist"
                >
                  Sign out
                </button>
              ) : null}
          ...
          <div className="flex items-center justify-between rounded-[12px] bg-mist px-4 py-4">
            <div>
            ...
            {fields.map((field) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center justify-between rounded-[12px] border border-midnight/8 px-4 py-4 text-left"
                >
                  <div>
          ...
          {currentUser.appRole !== "sponsor" && currentUser.appRole !== "admin" ? (
            <div className="rounded-[12px] border border-midnight/8 px-4 py-4">
              <div>
          ...
          <div className="rounded-[12px] border border-dashed border-midnight/14 bg-[rgba(12,73,90,0.03)] p-4">
            ...
            <p className="mt-2 text-sm text-midnight/64">
              ...
              {ticketing.isConfigured
                ? "Primary registration link is configured."
                : `Set NEXT_PUBLIC_EVENTBRITE_EVENT_URL or contact ${ticketing.supportEmail}.`}
            </p>
          </div>
        </article>
      </div>
