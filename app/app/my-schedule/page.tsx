...
export default function MySchedulePage() {
  ...
  return (
    <section className="flex min-h-[calc(100vh-10rem)] flex-col gap-4">
      ...
      >
        <article className="flex min-h-[140px] flex-col rounded-[28px] border border-midnight/8 bg-white p-4 shadow-card sm:min-h-[168px] sm:p-5">
          <p className="text-xs uppercase tracking-[0.24em] text-coral">Next up</p>
          {nextUp ? (
            <div className="mt-3 flex-1 rounded-[22px] bg-mist p-3 sm:p-4">
            ...
            <div className="mt-3 grid flex-1 place-items-center rounded-[22px] bg-mist px-5 text-center">
            ...
            <div className="mt-3 grid flex-1 content-start gap-2 overflow-visible md:overflow-y-auto md:pr-1 md:[scrollbar-gutter:stable]">
              {changedSessions.slice(0, 3).map((session) => (
                <Link
                  ...
                  className="rounded-[18px] bg-mist px-3 py-3 transition hover:bg-[#ece8dd] sm:px-4"
                >
      ...
      <div className="grid gap-4 xl:grid-cols-[1.1fr,0.9fr] xl:items-stretch">
        <div className="flex min-h-0 flex-col rounded-[30px] border border-midnight/8 bg-[#f9fafc] p-3 shadow-card md:h-[560px] md:min-h-[560px]">
          <div className="mb-3 rounded-[22px] border border-white/60 bg-white/80 px-4 py-3 backdrop-blur">
          ...
          <div className="flex-1 space-y-2.5 overflow-visible md:overflow-y-auto md:pr-1 md:[scrollbar-gutter:stable]">
            ...
            {!daySessions.length ? (
              <div className="grid min-h-[240px] place-items-center rounded-[22px] border border-dashed border-midnight/10 bg-white/55 px-6 text-center">
                <div>
                  <p className="text-[11px] font-bold uppercase tracking-[0.18em] text-coral">
                    No saved sessions
                  </p>
        ...
        <article className="flex min-h-0 flex-col rounded-[28px] border border-midnight/8 bg-white p-5 shadow-card md:h-[560px] md:min-h-[560px]">
            ...
            {recommendations.length ? (
              recommendations.map((session) => (
                <Link
                  key={session.id}
                  href={`/app/sessions/${session.slug}`}
                  className="rounded-[20px] border border-midnight/8 px-4 py-3 transition hover:border-midnight/16 hover:bg-mist"
                >
                  <p className="break-words font-display text-[15px] font-semibold leading-[1.2] text-midnight">
                    {session.title}
                  </p>
              ...
              <div className="grid flex-1 place-items-center rounded-[22px] bg-mist px-5 text-center">
                <p className="text-sm text-midnight/64">
        ...
        <div className="relative z-[1] pointer-events-none">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              <div className="break-words font-display text-[15px] font-semibold leading-[1.2] text-midnight">
                {session.title}
              </div>
              <p className="mt-1 break-words text-[12px] leading-5 text-midnight/55">
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            {sponsorName ? (
              <div className="inline-flex rounded-[4px] bg-[rgba(245,200,66,0.18)] px-[6px] py-[3px] text-[10px] font-bold uppercase tracking-[0.08em] text-[#8f6a02]">
                Sponsored · {sponsorName}
              </div>
            ) : null}

            {isUpdated ? (
              <div className="inline-flex rounded-[4px] bg-[rgba(26,92,182,0.12)] px-[6px] py-[3px] text-[10px] font-bold uppercase tracking-[0.08em] text-indigo">
                Updated
              </div>
            ) : null}

            {isConflict ? (
              <div className="inline-flex rounded-[4px] bg-[#ffe3e7] px-[6px] py-[3px] text-[10px] font-bold uppercase tracking-[0.08em] text-[#b91c2c]">
                Overlap
              </div>
            ) : null}
          </div>
