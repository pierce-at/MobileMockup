          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            {filteredSponsors.map((sponsor) => (
              <Link
                ...
                className="rounded-[14px] border border-midnight/8 bg-white p-5 shadow-card transition hover:translate-y-[-2px]"
              >
                <div className="grid h-16 w-16 place-items-center rounded-[12px] bg-midnight font-display text-xl font-semibold text-gold">
                  {sponsor.logo}
                </div>
                <div className="mt-4 flex flex-wrap gap-1.5">
                  <span className="rounded-full bg-[rgba(251,189,25,0.18)] px-3 py-1 text-[11px] font-semibold text-midnight">
                    {sponsor.tier}
                  </span>
                  {sponsor.track ? (
